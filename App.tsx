...
const AssetDisplay: React.FC<{ assets: AssetEntry }> = ({ assets }) => {
  ...
  const AssetRow = ({ icon: Icon, label, value, field }: { icon: any, label: string, value: string, field: string }) => (
    <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100 group transition-all hover:bg-white hover:shadow-sm">
      <div className="flex items-center gap-3">
        <div className="bg-brand-cyan/10 p-2 rounded-lg"><Icon className="w-4 h-4 text-brand-cyan" /></div>
        <div>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-none">{label}</p>
          <p className="text-sm font-mono font-bold text-slate-800 mt-1">{value || 'N/A'}</p>
        </div>
      </div>
      {value && (
        <button onClick={() => copy(value, field)} className="p-2 text-slate-300 hover:text-brand-cyan">
  ...
  return (
    <div className="mt-4 bg-white border-2 border-slate-100 rounded-2xl overflow-hidden shadow-sm">
      ...
      <div className="p-3 grid grid-cols-1 sm:grid-cols-2 gap-2 bg-white">
        ...
        <AssetRow icon={Monitor} label="Monitor" value={assets.monitor} field="monitor" />
...
const App: React.FC = () => {
    ...
    if (awaitingEmailForCustody) {
      ...
      setTimeout(() => {
        if (asset) {
          ...
          setMessages(prev => [...prev, { id: Date.now().toString(), role: 'assistant', content: `I couldn't find any equipment records for **${currentInput}**. Please verify the email or contact your local IT lead.` }]);
        }
        setIsTyping(false);
  ...
  const generateEmailBody = (status: 'RESOLVED' | 'UNRESOLVED') => {
    ...
    messages.forEach(m => {
      if (m.choices) return;
      const role = m.role === 'user' ? 'User' : 'SAM';
      let content = m.content.replace(/<img [^>]*src="([^"]*)"[^>]*>/gi, '\n[IMAGE]\n').replace(/<[^>]*>?/gm, '');
      body += `${role}: ${content}\n\n`;
    });
    return body;
  };
  ...
  const sendSupportEmail = () => {
    const history = generateEmailBody('UNRESOLVED');
    window.location.href = `mailto:itsupport@assemblyglobal.com?subject=IT Help [UNRESOLVED]&body=${encodeURIComponent(history)}`;
  };

  const sendResolvedEmail = () => {
    const history = generateEmailBody('RESOLVED');
    window.location.href = `mailto:itsupport@assemblyglobal.com?cc=essam.rabie@stagwellgtg.com&subject=IT Resolved&body=${encodeURIComponent(history)}`;
    setMessages(prev => [...prev, { id: Date.now().toString(), role: 'assistant', content: "Excellent! I've logged this session as resolved. Have a great day!" }]);
    setShowFeedback(false);
  };
    ...
    <div className="flex flex-col h-dvh bg-[#f8fafc] overflow-hidden">
      <header className="glass-header border-b border-slate-200 px-4 py-3 flex items-center justify-between sticky top-0 z-20 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setShowAbout(true)} className="p-2 text-slate-400 hover:text-brand-cyan transition-colors" title="About SAM"><Info className="w-5 h-5" /></button>
          <button onClick={() => setIsAdmin(!isAdmin)} className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold transition-all ${isAdmin ? 'bg-brand-dark text-white' : 'bg-white border border-slate-200'}`}>
        ...
        {showAbout && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-brand-dark/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl relative text-center">
              <button onClick={() => setShowAbout(false)} className="absolute top-4 right-4 p-2 text-slate-400 hover:text-red-500 transition-colors"><X className="w-5 h-5" /></button>
              <Logo size="md" />
              <h3 className="text-xl font-black text-brand-dark mt-4">About SAM Agent</h3>
              <p className="text-sm text-slate-600 mt-2">SAM is a smart IT Support Assistant designed to provide instant answers and hardware tracking for our employees.</p>
              
              <div className="mt-6 p-4 bg-slate-50 rounded-2xl border border-slate-100 text-left space-y-4">
                <div className="flex items-start gap-3">
                  <User className="w-4 h-4 text-brand-cyan mt-1 flex-shrink-0" />
                  <div>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-tight">Created By</p>
                    <p className="text-sm font-bold text-slate-800">Essam.rabie@stagwellgtg.com</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <MessageSquareText className="w-4 h-4 text-brand-cyan mt-1 flex-shrink-0" />
                  <div>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-tight">Feedback</p>
                    <p className="text-xs text-slate-600 font-medium">Reach out to us for any feedback or suggestions to improve the agent.</p>
                  </div>
                </div>
              </div>
              
              <button onClick={() => setShowAbout(false)} className="w-full mt-6 bg-brand-dark text-white py-4 rounded-xl font-bold hover:brightness-110 transition-all shadow-lg">Close</button>
            </div>
          </div>
        )}
        ...
        {isAdmin ? (isAdminAuthenticated ? <AdminPanel /> : (
            <div className="flex items-center justify-center min-h-[60vh] p-6 animate-in zoom-in-95">
              <div className="w-full max-w-sm bg-white p-8 rounded-3xl shadow-xl text-center space-y-6">
                <Lock className="w-10 h-10 text-brand-cyan mx-auto" />
                <h2 className="text-2xl font-black text-brand-dark">Admin Access</h2>
                <p className="text-sm text-slate-500">Authorized personnel only.</p>
                <form onSubmit={handleAdminLogin} className="space-y-4">
                  <input autoFocus type="password" value={adminPassInput} onChange={(e) => {setAdminPassInput(e.target.value); setAdminLoginError(false);}} placeholder="••••••••" className={`w-full p-4 bg-slate-50 border-2 rounded-2xl text-center text-lg tracking-widest outline-none focus:border-brand-cyan transition-all ${adminLoginError ? 'border-red-500 animate-shake' : 'border-slate-100'}`} />
                  <button type="submit" className="w-full bg-brand-dark text-white py-4 rounded-2xl font-bold shadow-lg hover:brightness-110 transition-all">Unlock</button>
                  {adminLoginError && <p className="text-red-500 text-xs font-bold animate-in fade-in">Incorrect password.</p>}
                </form>
              </div>
            </div>
          )) : (
          <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-8">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} animate-in slide-in-from-bottom-2`}>
                <div className={`flex items-start gap-3 max-w-[95%] md:max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className={`p-4 rounded-2xl text-sm leading-relaxed shadow-sm ${msg.role === 'user' ? 'bg-brand-dark text-white rounded-tr-none' : 'bg-white border border-slate-100 text-slate-700 rounded-tl-none'}`}>
                    {msg.role === 'assistant' && msg.isKBMatch && <div className="flex items-center gap-1.5 mb-2 text-brand-cyan font-bold text-[10px] uppercase border-b border-slate-50 pb-1.5"><ShieldCheck className="w-3.5 h-3.5" /> Database Match</div>}
                    <div className="rich-answer" dangerouslySetInnerHTML={{ __html: msg.content }} />
                    {msg.choices && <div className="flex flex-wrap gap-2 mt-4">{msg.choices.map(c => <button key={c} onClick={() => handleChoiceClick(c)} className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-full text-xs font-bold text-brand-cyan hover:bg-brand-cyan hover:text-white transition-all">{c}</button>)}</div>}
                    {msg.assetData && <AssetDisplay assets={msg.assetData} />}
                  </div>
                </div>
              </div>
            ))}
            {isTyping && <div className="flex items-start gap-3"><div className="bg-white border p-4 rounded-2xl flex gap-1.5 shadow-sm"><div className="w-1.5 h-1.5 bg-slate-300 rounded-full animate-bounce"></div><div className="w-1.5 h-1.5 bg-slate-300 rounded-full animate-bounce delay-100"></div><div className="w-1.5 h-1.5 bg-slate-300 rounded-full animate-bounce delay-200"></div></div></div>}
            {showFeedback && !isTyping && (
              <div className="bg-brand-cyan/5 border border-brand-cyan/10 rounded-3xl p-6 max-w-2xl mx-auto text-center space-y-6 shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-500">
                <h3 className="text-lg font-extrabold">Helpful response?</h3>
                <div className="flex flex-col gap-3 max-w-lg mx-auto">
                  <button onClick={() => setShowFeedback(false)} className="w-full bg-white border border-slate-200 text-slate-600 px-4 py-3 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-slate-50 shadow-sm transition-all"><MessageSquareText className="w-5 h-5" /> Ask something else</button>
                  <div className="grid grid-cols-2 gap-3">
                    <button onClick={sendResolvedEmail} className="bg-white border-2 border-brand-cyan text-brand-cyan py-3 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-brand-cyan hover:text-white transition-all shadow-sm"><ThumbsUp className="w-5 h-5" /> Yes</button>
                    <button onClick={sendSupportEmail} className="bg-brand-dark text-white py-3 rounded-2xl font-bold flex items-center justify-center gap-2 shadow-lg hover:brightness-110 transition-all"><ThumbsDown className="w-5 h-5" /> No</button>
                  </div>
                </div>
              </div>
            )}
        ...
        <footer className="bg-white border-t border-slate-200 px-4 py-6 shadow-2xl relative z-30">
          <div className="max-w-4xl mx-auto">
            <div className="relative">
              <input type="text" value={inputValue} onChange={e => setInputValue(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleSendMessage()} placeholder={awaitingAgencyName ? "Enter agency name..." : awaitingEmailForCustody ? "Enter corporate email..." : "Ask SAM about IT issues..."} className="w-full bg-slate-50 border-2 border-slate-200 rounded-2xl pl-5 pr-14 py-4 focus:outline-none focus:border-brand-cyan shadow-inner transition-all" />
              <button onClick={handleSendMessage} disabled={!inputValue.trim() || isTyping} className="absolute right-2 top-2 bg-brand-cyan text-white p-3 rounded-xl hover:bg-brand-cyan/90 disabled:opacity-30 transition-all shadow-md active:scale-95"><Send className="w-6 h-6" /></button>
            </div>
